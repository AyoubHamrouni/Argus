#!/usr/bin/env bash
# =============================================================================
# Argus Master Deployment Script (Linux/macOS)
# =============================================================================
# Single-command deploy for the entire Argus stack.
#
# Usage:
#   ./deploy-argus.sh           Deploy all services
#   ./deploy-argus.sh --stop    Tear down all services
#   ./deploy-argus.sh --status  Show service status
#
# Phases:
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  (no args)  Deploy full Argus stack"
...
ensure_env() {
    ...
    if [[ -f "$env_file" ]]; then
        ...
        cat > "$env_file" <<'ENVEOF'
# Auto-generated by deploy-argus.sh
...
health_check() {
    ...
    if $all_ok; then
        ...
        warn "Run './deploy-argus.sh --status' to check container states."
    fi
}
...
print_access_urls() {
    ...
    echo ""
    echo -e "${GREEN}${BOLD}Argus deployment complete.${RESET}"
    echo -e "Run '${CYAN}./deploy-argus.sh --stop${RESET}' to tear down all services."
}
