# =============================================================================
# Argus Master Deployment Script (Windows PowerShell)
# =============================================================================
# Single-command deploy for the entire Argus stack.
#
# Usage:
#   .\deploy-argus.ps1           Deploy all services
#   .\deploy-argus.ps1 -Stop     Tear down all services
#   .\deploy-argus.ps1 -Status   Show service status
#
# Phases:
...
# Colour helpers
# ---------------------------------------------------------------------------
function Write-Log    { param($Msg) Write-Host "[Argus] $Msg" -ForegroundColor Cyan }
...
# ---------------------------------------------------------------------------
if ($Help) {
    Write-Host "Usage: .\deploy-argus.ps1 [-Stop] [-Status] [-Help]"
    Write-Host "  (no flags)  Deploy full Argus stack"
...
function Show-Status {
    Write-Banner "Argus Service Status"
...
function Ensure-Env {
    ...
    if (Test-Path $envFile) {
        ...
        @"
# Auto-generated by deploy-argus.ps1
...
function Invoke-HealthCheck {
    ...
    if ($allOk) {
        ...
        Write-Warn "Some services not yet reachable. Run '.\deploy-argus.ps1 -Status' to check container states."
    }
}
...
function Print-AccessUrls {
    ...
    Write-Host ""
    Write-Host "Argus deployment complete." -ForegroundColor Green
    Write-Host "Run '.\deploy-argus.ps1 -Stop' to tear down all services." -ForegroundColor Gray
}
